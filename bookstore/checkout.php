...
<?php
	...
	if ($row) {
		...
		// Rest of your echo statements for other fields...
		echo "<tr><td>Date: </td><td>".$row['DatePurchase']."</td></tr>";
		echo "</blockquote>";
	} else {
		echo "No data found"; // Or handle the situation where $row is empty
	}
	...
	echo "<tr><td style='background-color: #ccc;'></td><td style='text-align: right;background-color: #ccc;''>Total Price: <b>Dt".$total."</b></td></tr>";
	echo "</table>";
	echo "</div>";
...
if(isset($_POST['submitButton'])){
	if (empty($_POST["name"])) {
		...
		if (!preg_match("/^[<a-zA-Z1-9></a-zA-Z1-9> ]*$/", $name)){
